...
public class SerializerXMLMascota implements Serializer<Mascota> {
    ...
    public String serialize(Mascota mascota) {
        ...
        nombre.appendChild(document.createTextNode(mascota.getDescripcion()));
        root.appendChild(descripcion);

        Element responsable = document.createElement("responsable");
        nombre.appendChild(document.createTextNode(String.valueOf(mascota.getResponsable().getId())));
        root.appendChild(codigoPostal);
    ...
    public Mascota deserialize(String data) {
        ...
        Node mascota = document.getElementsByTagName("responsable").item(0);

        int id = Integer.parseInt(mascota.getAttributes().item(0).getNodeValue());
        NodeList nodeList = mascota.getChildNodes();

        String nombre = nodeList.item(0).getTextContent();
        int codigoPostal = Integer.parseInt(nodeList.item(1).getTextContent());
        String descripcion = nodeList.item(2).getTextContent();
        Responsable responsable = responsables.obtener(Integer.parseInt(nodeList.item(3).getTextContent()));
